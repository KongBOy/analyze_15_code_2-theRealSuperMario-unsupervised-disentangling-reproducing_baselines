...
class Test_model:
    def test_forward(self):
        ...
        image_batch_tiled = ops_pt.tile_nd(image_batch, [2, 1, 1, 1]).cuda()
        arg = ModelArgs(bn=1)
    ...
    def test_adversarial(self):
        ...
        coord, vector = tps.make_input_tps_param(tps_param_dic)
        t_images, t_mesh = tps.ThinPlateSpline(
            image_batch_tiled, coord, vector, model.in_dim, model.n_c
        )
        image_in, image_rec = ops_pt.prepare_pairs(
            t_images,
            model.reconstr_dim,
            train=True,
            static=False,
            contrast_var=model.contrast_var,
            brightness_var=model.brightness_var,
            saturation_var=model.saturation_var,
            hue_var=model.hue_var,
            p_flip=model.p_flip,
        )
        model = model.cuda()
